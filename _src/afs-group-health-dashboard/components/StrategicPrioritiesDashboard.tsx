...
import { Flag, Loader2, ChevronLeft, ChevronRight, User } from 'lucide-react';
...
async function fetchAsanaInitiatives(): Promise<{ tasks: any[]; sprintOptions: string[] }> {
    ...
    // Match GROUP exactly: the board also has GAF / FORCE / REVEL / BHAG + CRITICAL NUMBERS
    // sections, and a loose includes() would collide with none of them today but is one
    // renamed section away from silently pulling the wrong brand's priorities.
    const groupSection = (await secRes.json()).data.find((s: any) => s.name.trim().toUpperCase() === "GROUP");
    if (!groupSection) throw new Error("GROUP section not found");

    const tasksRes = await fetch(`${ASANA_PROXY}/sections/${groupSection.gid}/tasks?opt_fields=name,completed,due_on,assignee.name,permalink_url,custom_fields,custom_fields.name,custom_fields.display_value,custom_fields.enum_options,parent,parent.name`);
            ...
            <header className="sticky top-0 z-50 backdrop-blur-xl bg-white/70 border-b border-white/20 shadow-lg">
                <div className="mx-auto max-w-5xl px-4 py-3 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-[#fdece5] rounded-xl text-[#F26422]">
                            <Flag size={24} />
                        </div>
                        <div>
                            <h1 className="text-lg md:text-xl font-bold tracking-tight text-[#0E2A44]">Strategic Priorities</h1>
            ...
            <div className="max-w-5xl mx-auto p-4 md:p-6 mt-4">
                        ...
                        <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="text-center min-w-[180px]">
                                    ...
                                    {isCurrentSprint && (
                                        <div className="text-[10px] font-bold text-[#F26422] uppercase tracking-widest">Current Quarter</div>
                                    )}
                                </div>
                                <button
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {sprintOptions.map(sprint => (
                                <button
                                    ...
                                    } ${sprint === getCurrentSprint() && selectedSprint !== sprint ? 'ring-1 ring-[#F26422]/30' : ''}`}
                                >
                                    {sprint}
                                </button>
                            ))}
                ...
                {isAsanaLoading ? (
                    <div className="flex flex-col items-center justify-center p-12 bg-white rounded-2xl border border-gray-200 shadow-sm mt-8">
                        <Loader2 className="w-8 h-8 text-[#F26422] animate-spin mb-4" />
                            ...
                            return (
                                <div key={task.gid} className="bg-white rounded-[6px] shadow-sm border border-[#e8e8e9] p-4 transition-all hover:bg-gray-50/50 flex flex-col gap-3 group relative">
                                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                                        <div className="flex items-start md:items-center gap-3 flex-1 min-w-0">
                                            ...
                                            {task.assigneeName ? (
                                                <div className="w-8 h-8 rounded-full bg-[#fbdca7] text-[#865d21] flex items-center justify-center text-xs font-bold shrink-0 mt-1 md:mt-0" title={task.assigneeName}>
                                                    {(task.assigneeName.includes(' ') ? task.assigneeName.split(' ').map((n: string) => n[0]).join('').substring(0, 2) : task.assigneeName.substring(0, 2)).toUpperCase()}
                                                </div>
                                            ) : (
                                                /* Unassigned reads as a deliberate gap, not a rendering fault. */
                                                <div className="w-8 h-8 rounded-full border border-dashed border-gray-300 text-gray-400 flex items-center justify-center shrink-0 mt-1 md:mt-0" title="Unassigned">
                                                    <User size={14} />
                                                </div>
                                            )}
                                        ...
                                        {task.permalink_url ? (
                                            <a href={task.permalink_url} target="_blank" rel="noopener noreferrer" className="self-end md:self-center ml-auto md:ml-2 text-gray-400 hover:text-[#F26422] p-2 rounded hover:bg-[#fdece5] transition-colors" title="View in Asana">
