...
export default function CallInsightsDashboard() {
  const { data, loading, error } = useCallData(CSV_URL);
  ...
  const filtered = useMemo(() => {
    return rawDataInDateRange.filter((row) => {
        ...
        aircall_url: (() => {
          if (row.call_id) return `https://dashboard-v2.aircall.io/calls/${row.call_id}`;
          return "";
        })(),
  ...
  const directionData = useMemo(() => {
      ...
      return counts.map((c, i) => ({ name: c.name, value: c.count, fill: i === 0 ? GAF_COLORS.orange : GAF_COLORS.darkGrey }));
  }, [filtered]);

  const segmentData = useMemo(() => {
      const counts = getFrequency(filtered.map(r => r.customer_segment).filter(Boolean));
      return counts.map((c, i) => ({ name: c.name, value: c.count, fill: i % 2 === 0 ? GAF_COLORS.blue : GAF_COLORS.coolGrey }));
  }, [filtered]);

  // Pagination Logic for Main Table
  const paginatedCalls = useMemo(() => {
      const start = (callsPage - 1) * CALLS_PER_PAGE;
      const slice = filtered.slice(start, start + CALLS_PER_PAGE);
      
      // Safely parse raw_json only for the current page to avoid performance issues
      return slice.map(call => {
        let s3_url = "";
        let short_url = "";
        
        try {
          if (call.raw_json) {
            const j = JSON.parse(call.raw_json);
            if (j.call) {
              if (j.call.recording) s3_url = j.call.recording;
              if (j.call.recording_short_url) short_url = j.call.recording_short_url;
              if (j.call.public_url) short_url = j.call.public_url;
            }
          }
        } catch (e) {
          // Ignore parse errors
        ...
        <section className="space-y-4 md:space-y-6">
           ...
           <div className="space-y-6">
              <div className={CARD_CLASS}>
                  ...
                  <ThemeSection title="Functional" data={functionalThemes} colorIndex={0} loading={loading} onJobClick={(job) => addFilter("JOB", job)} />
              </div>

              <div className={CARD_CLASS}>
                  <h3 className="text-sm md:text-base font-black mb-4 text-pink-600 uppercase tracking-tight">Emotional Jobs</h3>
                  <ThemeSection title="Emotional" data={emotionalThemes} colorIndex={2} loading={loading} onJobClick={(job) => addFilter("JOB", job)} />
              </div>
              ...
              {socialThemes.length > 0 && (
                <div className={CARD_CLASS}>
                    ...
                    <ThemeSection title="Social" data={socialThemes} colorIndex={4} loading={loading} onJobClick={(job) => addFilter("JOB", job)} />
                </div>
              )}
           </div>
        </section>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-gray-50 border-b border-gray-100">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-xs font-bold text-gray-500 uppercase">Links</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={i} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 align-top">
                                        <div className="flex flex-col gap-2">
                                            {/* Priority 1: Public Short URL (Permanent, No Login) */}
                                            {call.short_url ? (
                                                <a 
                                                    href={call.short_url} 
                                                    target="_blank" 
                                                    rel="noopener noreferrer"
                                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-green-50 text-green-600 rounded-lg text-xs font-bold hover:bg-green-100 transition-colors w-fit"
                                                    title="Public recording link (No login required)"
                                                >
                                                    <Play size={12} fill="currentColor" /> Public Link
                                                </a>
                                            ) : null}

                                            {/* Priority 2: Direct S3 URL (No Login, but Expires) */}
                                            {call.s3_url ? (
                                                <a 
                                                    href={call.s3_url} 
                                                    target="_blank" 
                                                    rel="noopener noreferrer"
                                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 text-orange-600 rounded-lg text-xs font-bold hover:bg-orange-100 transition-colors w-fit"
                                                    title="Direct MP3 link (May expire after 1 hour)"
                                                >
                                                    <Play size={12} fill="currentColor" /> Direct MP3
                                                </a>
                                            ) : null}

                                            {/* Fallback: Aircall Asset URL (Requires Login) */}
                                            {(!call.short_url && !call.s3_url && call.mp3_url) && (
                                                <a 
                                                    href={call.mp3_url} 
                                                    target="_blank" 
                                                    rel="noopener noreferrer"
                                                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 text-orange-600 rounded-lg text-xs font-bold hover:bg-orange-100 transition-colors w-fit"
                                                    title="Aircall asset link (Requires Aircall login)"
                                                >
                                                    <Play size={12} fill="currentColor" /> Play MP3
                                                </a>
                                            )}

                                            {call.aircall_url && (
                                                <a 
                                                    href={call.aircall_url} 
                                                    target="_blank" 
                                                    rel="noopener noreferrer"
                                                    ...
                                                    title="View call details in Aircall Dashboard"
                                                >
                                                    <ExternalLink size={12} /> Aircall
                                                </a>
                                            )}
