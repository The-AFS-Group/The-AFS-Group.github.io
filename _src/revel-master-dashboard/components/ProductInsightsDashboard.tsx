...
import { fetchProductInsightsData, getCachedData, isCacheStale } from '../services/dataService';
...
import { 
    ...
    Eye, Package, SlidersHorizontal, ChevronRight, ChevronLeft, Bot, Send, User, ChevronDown, MessageSquare, 
    CheckCircle2, Users, Lightbulb, Target, BookOpen, Truck, Sword, Info, Check, Search, List, Pencil, X, ExternalLink, Play
} from 'lucide-react';
import { GAF_COLORS } from '../constants';
import { GoogleGenAI } from "@google/genai";
import { UpdatingBadge } from './UpdatingBadge';

const DATA_SOURCE_URL = "https://docs.google.com/spreadsheets/d/1hygNBN9A662jRSAhLAq3CfHrmcQhZrbMFl68GSEAfNM/edit?gid=132632060#gid=132632060";
...
const REVEL_CORE_CATEGORIES = ["Combination Sauna", "Ice Bath", "Infrared Saunas", "Traditional Saunas"];

const TAB_DEFAULTS: Record<string, FilterConfig> = {
    underperform: { gpThreshold: 7, inventoryThreshold: 3, viewsThreshold: 1200, priceThreshold: 400, estRevenueThreshold: 0, defaultSelectedTypes: REVEL_CORE_CATEGORIES },
    traffic: { gpThreshold: 25, inventoryThreshold: 3, viewsThreshold: 700, priceThreshold: 600, estRevenueThreshold: 0, defaultSelectedTypes: REVEL_CORE_CATEGORIES },
    high: { gpThreshold: 30, inventoryThreshold: 0, viewsThreshold: 250, priceThreshold: 0, estRevenueThreshold: 0, defaultSelectedTypes: REVEL_CORE_CATEGORIES },
    risks: { gpThreshold: 0, inventoryThreshold: 0, viewsThreshold: 0, priceThreshold: 0, estRevenueThreshold: 0, defaultSelectedTypes: [] },
    all: { gpThreshold: 0, inventoryThreshold: 0, viewsThreshold: 0, priceThreshold: 0, estRevenueThreshold: 0, defaultSelectedTypes: [] }
};
...
const ProductTable: React.FC<ProductTableProps> = ({ 
    ...
    const handleSort = (col: string) => {
        ...
        switch(col) {
            ...
            case 'Est Revenue (30d)': key = 'estRevenue30d'; break;
            ...
            case 'PreOrder Inv': key = 'preOrderInventory'; break;
    ...
    const sortedData = useMemo(() => {
        return [...data].sort((a, b) => {
            const valA = a[sortCol as keyof ProductPerformance];
            const valB = b[sortCol as keyof ProductPerformance];
    ...
    const renderCell = (row: ProductPerformance, col: string) => {
        ...
        switch(col) {
            ...
            case 'Est Revenue (30d)': return <span className={baseClass}>{formatCurrency(row.estRevenue30d)}</span>;
            case 'GP/Visit (30d)': {
            ...
            case 'PreOrder Inv': return row.preOrderInventory > 0 ? <span className="text-blue-600 text-sm">{row.preOrderInventory}</span> : <span className="text-gray-400 text-sm">-</span>;
            ...
            case 'GAF URL': {
                const url = row.url.startsWith('http') ? row.url : `https://revelsaunas.com.au${row.url.startsWith('/') ? '' : '/'}${row.url}`;
                return (
                    <a 
                        href={url} 
                        target="_blank" 
    ...
    const handleSend = async () => {
        ...
        try {
            const summaryStats = {
                ...
                totalGP: contextData.reduce((acc, curr) => acc + (curr.grossProfit || 0), 0).toFixed(0),
                vendors: [...new Set(contextData.map(d => d.vendor))].slice(0, 10).join(", ")
            };
            
            const dataSample = contextData.slice(0, 50).map(p => ({
                sku: p.sku, name: p.name, gpVisit: p.gpPerVisit30d, views: p.pageViews30d, inv: p.inventory, vendor: p.vendor
            }));
            ...
            const prompt = `
                ...
                Data Sample (Top 50): ${JSON.stringify(dataSample)}
                User Question: ${userMsg}
                Answer concisely.
            `;

            // Always use ai.models.generateContent and access .text property.
            ...
            <div className="p-4 border-t border-gray-200 bg-white sticky bottom-0">
                <div className="relative">
                    <input 
                        ...
                        className="w-full pl-4 pr-10 py-3 rounded-xl border border-gray-300 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 text-sm bg-white text-gray-900"
                    />
                    <button 
                        onClick={handleSend}
...
export default function ProductInsightsDashboard() {
  ...
  const cachedData = getCachedData<ProductInsightsData>(cacheKey);
  const [data, setData] = useState<ProductInsightsData | null>(cachedData || null);
  ...
  const [selectedProductTypes, setSelectedProductTypes] = useState<string[]>(TAB_DEFAULTS.all.defaultSelectedTypes || []);
  ...
  const handleTabChange = (tabId: typeof activeTab) => {
      ...
      if (TAB_DEFAULTS[tabId]) {
          ...
          if (TAB_DEFAULTS[tabId].defaultSelectedTypes) {
             setSelectedProductTypes(TAB_DEFAULTS[tabId].defaultSelectedTypes!);
          } else {
  ...
  useEffect(() => {
    const load = async (force = false) => {
      ...
      setIsUpdating(true);
      const result = await fetchProductInsightsData(force);
      setData(result);
      setIsUpdating(false);
    };
    load();
  ...
  const filteredData = useMemo(() => {
      ...
      let result = data.performance.filter(p => {
          if (!p.sku || p.sku.trim() === "" || p.sku === "#N/A") return false;
      ...
      switch(activeTab) {
          case 'underperform':
              // UNDERPERFORMING: GP/Visit: < $5, Views (30d): > 1,200
          ...
          case 'traffic':
              // NEEDS TRAFFIC: GP/Visit: > $25, Views (30d): < 700
          ...
          case 'high':
              // HIGH PERFORMERS: GP/Visit: > $30, Views (30d): > 250
              return result.filter(p => p.pageViews30d > config.viewsThreshold && p.gpPerVisit30d > config.gpThreshold);
          case 'risks':
              return result.filter(p => p.enoughStockRunRate === 'NO' && p.discontinued === false && p.gpPerVisit30d >= config.gpThreshold && p.inventory >= config.inventoryThreshold && p.pageViews30d >= config.viewsThreshold && p.currentPriceIncGst >= config.priceThreshold && p.estRevenue30d >= config.estRevenueThreshold);
          case 'all':
              return result.filter(p => p.gpPerVisit30d >= config.gpThreshold && p.inventory >= config.inventoryThreshold && p.pageViews30d >= config.viewsThreshold && p.currentPriceIncGst >= config.priceThreshold && p.estRevenue30d >= config.estRevenueThreshold);
  ...
  const logicDescription = useMemo(() => {
      switch(activeTab) {
          ...
          case 'high': return `GP/Visit > $${config.gpThreshold}, Views > ${config.viewsThreshold}`;
  ...
  const columns = useMemo(() => [
      'SKU', 'Name', 'Est Revenue (30d)', 'GP/Visit (30d)', 'Qty (30d)', 'Views (30d)', 'GP/Visit (3m)', 'Qty (3m)', 'Views (3m)',
      'Inv', 'Days Left', 'Run Rate OK?', 'PreOrder Inv', 'Next ETA',
      'Type', 'RRP', 'Sell Price (Inc GST)', 'Cost (Ex)', 'GP %', 'Vendor', 'Discontinued', 'Comp. Price', 'GAF URL'
  ], []);
            ...
            <div className="mx-auto max-w-[1920px] px-4 py-3 md:py-4 flex items-center justify-between">
                <div>
                    <div className="flex items-center gap-2">
                        <h1 className="text-lg md:text-xl font-bold tracking-tight text-gray-900">Product Strategy</h1>
                ...
                <button onClick={() => setShowChat(!showChat)} className={`flex items-center gap-2 px-3 py-2 md:px-4 md:py-2.5 rounded-xl transition-all ${showChat ? 'bg-orange-100 text-orange-700' : 'bg-gray-900 text-white hover:bg-gray-800 shadow-sm'}`}>
                    <Bot size={18} />
        ...
        <main className="max-w-[1920px] mx-auto px-2 md:px-4 py-4 md:py-6 space-y-4 md:space-y-6 w-full flex-1 flex flex-col min-h-0 overflow-hidden">
            <div className="flex-shrink-0 flex flex-col xl:flex-row gap-4 justify-between items-start xl:items-center">
                <div className="flex flex-col md:flex-row items-stretch md:items-center gap-3 w-full xl:w-auto">
                    ...
                    <div className="relative w-full md:w-64">
                        <input type="text" placeholder="Search SKU..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-gray-300 focus:outline-none focus:ring-1 focus:ring-orange-500 text-xs md:text-sm shadow-sm bg-white text-gray-900" />
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                    </div>
                </div>
                <button onClick={() => setShowFilters(!showFilters)} className="flex items-center gap-2 text-xs md:text-sm font-semibold text-gray-600 hover:text-orange-600 transition-colors bg-white px-3 py-2 rounded-lg border border-gray-200">
                ...
                <div className="flex-shrink-0 bg-white p-4 md:p-6 rounded-xl border border-gray-200 shadow-sm animate-in slide-in-from-top-2">
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-6">
                        {[
                            ...
                            { label: 'Est Rev ($)', key: 'estRevenueThreshold' }
                        ].map(f => (
                            <div key={f.key}>
                                <label className="text-[10px] md:text-xs font-bold text-gray-500 mb-1 block uppercase tracking-tight">{f.label}</label>
                                <input type="number" value={(config as any)[f.key]} disabled={f.disabled} onChange={(e) => setConfig({...config, [f.key]: Number(e.target.value)})} className="w-full p-2 border border-gray-300 rounded-lg text-xs md:text-sm focus:ring-1 focus:ring-orange-500 outline-none disabled:bg-gray-50 bg-white text-gray-900" />
                            </div>
                        ))}
                        <div className="col-span-2 md:col-span-3 lg:col-span-1">
                            ...
                            <div className="border border-gray-300 rounded-lg p-2 h-24 overflow-y-auto bg-gray-50/50 custom-scrollbar">
                                {uniqueProductTypes.map(type => (
                                   <div key={type} className="flex items-center gap-2 mb-1 cursor-pointer" onClick={() => setSelectedProductTypes(prev => prev.includes(type) ? prev.filter(t => t !== type) : [...prev, type])}>
            ...
            <div className="flex-1 flex flex-col min-h-0">
                 <div className="flex-shrink-0 flex flex-col md:flex-row justify-between items-start md:items-center mb-3 gap-2 px-1">
                    <div>
                        <h2 className="text-sm md:text-lg font-bold text-gray-900 leading-tight">{activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Products</h2>
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] md:text-sm font-bold text-gray-900 bg-orange-100 text-orange-700 px-2 py-1 rounded-lg">{filteredData.length} items</span>
                    </div>
                 </div>
                 <div className="relative flex-1 min-h-0">
                    <UpdatingBadge isUpdating={isUpdating} />
                    <ProductTable key={activeTab} data={filteredData} columns={columns} onPriceChange={handlePriceUpdate} defaultSortCol={activeTab === 'risks' ? 'estRevenue30d' : activeTab === 'underperform' ? 'pageViews30d' : 'grossProfit'} />
                 </div>
            </div>
        </main>
    </div>
